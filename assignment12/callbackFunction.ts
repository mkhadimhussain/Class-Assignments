...
function performOperator(a: number, b: number, callback: (result: number) => void) {
    const result = a + b;
    callback(result);
}
...
function favFruit(fruit: ()=> void) {    
    console.log(`My favourite Fruit is: ${fruit()}`);
    // fruit();
}

function fruit() {
    let myFruit: string[] = ["Mango", "Apple"]
    return myFruit;
}
